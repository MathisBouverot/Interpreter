...
| [< ' ('0' .. '9' | '.' as head); stream >] -> begin
    let buff = Buffer.create 10 in
      Buffer.add_char buff head;
      let read_dot, read_digit = 
        (if head = '.' then true, false else false, true) in
...
and lex_number buff read_dot read_digit =
  parser
| [< ' ('0' .. '9' as head); stream >] -> 
  Buffer.add_char buff head;
...
| [< ''.'; stream >] ->
  Buffer.add_char buff '.';
  if read_dot = true then 
    raise (Parse_error "a number can't contain more than a single '.'")
  else lex_number buff true read_digit stream
| [< stream = lex >] -> 
  if read_dot then
    [< 'Token.Float (float_of_string (Buffer.contents buff)); stream >]
